...
@NamedQueries({
        ...
        @NamedQuery(name = PortEntity.CLOSEST_PORT, query = "select new fish.focus.uvms.spatial.service.dto.PortDistanceInfoDto(p, distance(p.geom, :point, false) as dist) from PortEntity p where enabled=true AND dWithin(p.geom, :point, 22224.0, false) = true order by dist "),
